                ...
                <table width="100%" border="1" cellpadding="0" cellspacing="0" style="border-collapse:collapse;" bordercolor="#000000" class="tablesorter" id="sort_table">
                  ...
                  <?php					
					...
					if($_REQUEST[fname]!='' && $_REQUEST[stid]=='' && $_REQUEST[mobile]=='' && $_REQUEST[email]==''){
						$condition = "s.family_name LIKE '$_REQUEST[fname]%' OR s.family_name1 LIKE '$_REQUEST[fname]%' OR s.first_name LIKE '%$_REQUEST[fname]%' OR s.first_name LIKE '%$_REQUEST[fname]%'";
					...
					$condition = $condition."And admin_status='Pending' And s.id=c.student_id";
					
					//$condition = $dbf->getSearchStrings($_REQUEST["fname"],$_REQUEST["stid"],$_REQUEST["mobile"],$_REQUEST["email"], $centre_id,"s.*","And s.centre_id='$centre_id' And s.id=c.student_id");
					//End 4.					
					//echo var_dump($dbf->fetchOrder('student s, student_cancel c', $condition ,"c.dated desc"));
					//echo $condition;
