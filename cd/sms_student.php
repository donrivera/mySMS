...
<select name="student_2" id="student_2" style="border:solid 1px; border-color:#999999;background-color:#ECF1FF; width:192px;" onChange="show_student_mobile();">
    ...
    <?php
	foreach($dbf->fetchOrder('student s,student_moving m',"s.id=m.student_id And s.centre_id='$_SESSION[centre_id]' And m.status_id='$status_id'","","m.student_id","m.student_id") as $move){
		...
		if($_REQUEST['opval']=="Enquiry" || $_REQUEST['opval']=="Potential" || $_REQUEST['opval']=="Legally Critical" || $_REQUEST['opval']=="Cancelled - Refunded"){
			...
			?>
            <option value="<?php echo $res_student['id'];?>"><?php echo $res_student['first_name'];?> <?php echo $dbf->printStudentName($res_student['id']);?></option>
            <?php
			}
...
<select name="student_2" id="student_2" style="border:solid 1px; border-color:#999999;background-color:#ECF1FF; width:192px;" onChange="show_student_mobile();">
	...
	?>
	<option value="<?php echo $move['id'];?>"><?php echo $move['first_name'];?> <?php echo $dbf->printStudentName($move['id']);?></option>
	<?php } ?>
</select>
<?php } ?>
...
<select name="student_2" id="student_2" style="border:solid 1px; border-color:#999999;background-color:#ECF1FF; width:192px;"  onChange="show_teacher_mobile();">
    ...
    <?php
	foreach($dbf->fetchOrder('teacher_centre',"centre_id='$_SESSION[centre_id]'","","","") as $teacher){
		$teacher_name = $dbf->strRecordID("teacher", "*", "id='$teacher[teacher_id]'");
	?>
...
<select name="student_2" id="student_2" style="border:solid 1px; border-color:#999999;background-color:#ECF1FF; width:192px;"  onChange="show_staff_mobile();">
    ...
    <?php
	foreach($dbf->fetchOrder('user',"user_type <> 'Administrator' And user_type <> 'Teacher' And center_id='$_SESSION[centre_id]'","","","") as $staff){
	?>
