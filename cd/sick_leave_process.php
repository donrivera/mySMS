...
$cd_name = $dbf->getDataFromTable("user","user_name","id='$_SESSION[id]'");
...
$email_msg = str_replace('%cd_name%',$cd_name,$email_msg);	
...
$subject = str_replace('%status%',$status,$subj);
	...
	if($sms_gateway["status"]=='Enable')
	{
		...
		$sms = $_REQUEST['sms'];
		$sel_teacher_option=$_POST['option'];
		switch($sms)
		{
			case '1':	{
							//$sms_Message = $optionmsg;
							switch($sel_teacher_option)
							{	
								case '1':	{
												$Message="Test"; //$dbf->getDataFromTable("sms_templete","contents","id='42'");
												
												
											}break;
								case '2':	{
												$sms_cont = $dbf->getDataFromTable("sms_templete","contents","id='40'");
												//$search = array('%teacher%');//$replace = array($teacher);
