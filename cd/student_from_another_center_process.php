...
if($_REQUEST['action']=='update'){
	...
	$centre_from = $dtls["centre_from"];
	$centre_to = $dtls["centre_to"];
	
	$from_id = $dtls["from_id"];
	$to_id = $dtls["to_id"];
	
	$string="final_status='$_REQUEST[status]',final_comment='$comm'";
	$dbf->updateTable("transfer_different_centre",$string,"id='$tran_id'");
	
	if($_REQUEST["status"] == 'Approved'){
		
		//If group has been choose by Student Advisor
		if($from_id > 0 || $to_id > 0){
			...
			if($group > 0){
				...
				foreach($dbf->fetchOrder('transfer_different_centre_dtls',"parent_id='$tran_id'") as $transfer){
			...
			if($group > 0){
				...
				if($num_student == 0){
					...
					foreach($dbf->fetchOrder('transfer_different_centre_dtls',"parent_id='$tran_id'") as $transfer){
						...
						//Update in ENROLLED Table
						$string="centre_id='$centre_id'";
						$dbf->updateTable("student_enroll",$string,"student_id='$id1'");
						
						//Update in Fees Table
						$string="centre_id='$centre_id'";
						$dbf->updateTable("student_fees",$string,"student_id='$id1'");
					}
					...
					if($no_unit_finined > 0){
						...
						foreach($dbf->fetchOrder('transfer_different_centre_dtls',"parent_id='$tran_id'") as $transfer){				
							...
							//Update in ENROLLED Table
							$string="centre_id='$centre_id'";
							$dbf->updateTable("student_enroll",$string,"student_id='$id2'");				
							
							//Update in Fees Table
							$string="centre_id='$centre_id'";
							$dbf->updateTable("student_fees",$string,"student_id='$id2'");
						}
						//=====================================
				  ...
				  <tr>
					<td align="center" valign="top">&nbsp;</td>
						...
						foreach($dbf->fetchOrder('transfer_different_centre_dtls',"parent_id='$tran_id'") as $transfer){
							...
							//Update in ENROLLED Table
							$string="centre_id='$centre_id'";
							$dbf->updateTable("student_enroll",$string,"student_id='$id3'");
							
							//Update in Fees Table
							$string="centre_id='$centre_id'";
							$dbf->updateTable("student_fees",$string,"student_id='$id3'");
						}
						//=====================================
			...
			foreach($dbf->fetchOrder('transfer_different_centre_dtls',"parent_id='$tran_id'") as $transfer){
				...
				$res_inv = $dbf->strRecordID("centre","invoice_from","id='$centre_to'");
				$res_stu = $dbf->strRecordID("student_enroll","MAX(invoice_sl)","centre_id='$centre_to'");
				//Check serial no exist or not
				if($res_stu["MAX(invoice_sl)"] == '0'){
					$inv_no = "1";
				}else{
					$inv_no = $res_stu["MAX(invoice_sl)"]+1;
				}
				//Invoice No
				$inv_sl = $inv_no;
				$inv_no = date("y").date("m").$res_inv["invoice_from"].str_pad($inv_no, 5, "0", STR_PAD_LEFT);				
				//End											
								
				//Moving History table
				$date_time = date('Y-m-d H:i:s A');
					
				// If from group choose [Waiting]
				if($from_id > 0){
					
					//Get course id from transfer details table
					$course_id = $dbf->getDataFromTable("student_group", "course_id", "id='$from_id'");
					
					$string_st="student_id='$id1',status_id='3',course_id='$course_id',date_time='$date_time',user_id='$_SESSION[id]'"; //Waiting Status		
					$dbf->insertSet("student_moving",$string_st);
					
					$string2="student_id='$id1',course_id='$course_id',date_time='$date_time',user_id='$_SESSION[id]',status_id='3'";
					$dbf->insertSet("student_moving_history",$string2);					
									
				// [Potential]
				}else{
					
					$string_st="student_id='$id1',status_id='2',user_id='$_SESSION[id]'"; //Potential Status
					$dbf->insertSet("student_moving",$string_st);
										
					$string2="student_id='$id1',date_time='$date_time',user_id='$_SESSION[id]',status_id='2'";
					$dbf->insertSet("student_moving_history",$string2);			
				}
...
<script type="text/javascript">
	self.parent.location.href='student_from_another_center_manage.php';
