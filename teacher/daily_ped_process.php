...
$attendance_per_unit=$_POST['atendance_per_unit'];
...
for($u=1;$u<=$res_group['units'];$u++)
{
	...
	if(empty($ped_units))
	{
		if($u==$unit)
		{	#echo "p_unit1".$u."<BR/>";
			$string="ped_id='$ped_id',teacher_id='$uid', group_id='$_POST[cmbgroup]',course_id='$course_id', units='$unit',dated='$attend_date',material_overed='$mc',homework='$hw'";
			$dbf->insertSet("ped_units",$string);
		}
		else
		{	#echo "p_unit2".$u."<BR/>";
			$ped_units=$dbf->genericQuery("SELECT * FROM ped_units WHERE units='$u' AND teacher_id='$uid' and group_id='$_POST[cmbgroup]' and course_id='$course_id'");
			$string="ped_id='$ped_id',teacher_id='$uid', group_id='$_POST[cmbgroup]',course_id='$course_id',units='$u'";
			if(empty($ped_units)){$dbf->insertSet("ped_units",$string);}
		}
	}
	else
	{
		foreach($attendance_per_unit as $a=>$a_value):
			$hw=$homework[$a];
			$mc=$material_covered[$a];
			#echo $a_value.$hw.$mc."<BR/>";
			$string="dated='$attend_date',material_overed='$mc',homework='$hw'";
			$dbf->updateTable("ped_units",$string,"teacher_id='$uid' AND units='$a_value' and group_id='$_POST[cmbgroup]' and course_id='$course_id'");
		endforeach;
	}
}
foreach($_POST[student_id] as $s=>$s_value)
{
	...
	$get_attendance_per_day=$res_group['units']/$res_group['unit_per_day'];
