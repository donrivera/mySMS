...
$today = date('Y-m-d');
$today = date('Y-m-d',strtotime(date("Y-m-d", strtotime($today)) . "-14 day"));
...
foreach($dbf->fetchOrder('student_fees',"fee_date='$today' And status='0'","","","") as $group){
	...
	if($student_mobile["student_mobile"] != ''){
		...
		//$msg = "Dear ".$student_mobile["first_name"].", your payment of ".$group["fee_amt"]." for Berlitz ".$course["name"]." is past due. Please expedite payment or risk of further penalties.";
		
		$sms_cont = $dbf->getDataFromTable("sms_templete","contents","id='19'");
