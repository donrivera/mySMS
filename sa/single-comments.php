...
if($_REQUEST["action"] == "send"){
	...
	if($comments != ''){
		...
		$is_save = $dbf->countRows("student_comment", "student_id='$student_id' And user_id='$users_id' And comments='$comments'");
		if($is_save == 0){
			
			$string="student_id='$student_id',user_id='$users_id',comments='$comments',date_time='$mydt'";
	...
	header("Location:single-comments.php?student_id=$student_id#focusme");
}
?>
...
function save(){
	...
	ajaxRequest.open("GET", 'view_student_comments_history_from_manage_save.php?action=send&msg='+msg+'&ids='+ <?php echo $student_id;?>, true);
...
function show(){
	...
	ajaxRequest.open("GET", "view_student_comments_history_from_manage_show.php?ids=<?php echo $student_id;?>", true);
	ajaxRequest.send(null);
}
