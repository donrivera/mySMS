...
class mydb
{
	...
	function backupDatabase($name,$tables = '*')
	{
		...
		if (file_exists($fileName))
		{
			...
			$outputArchive = $zipName.'.zip'; 
			$archive = new PclZip($outputArchive); //name target zip
			...
			$resultLink = "<a href='$outputArchive' target='_blank'>click here to DOWNLOAD DATABASE</a> or <a href='".$_SERVER['PHP_SELF']."?cTask=backUpDB&cTask2=removeBckDB&file=$outputArchive'>remove backup database</a>.";
    	}
		echo $resultLink;
	}
	...
	function restoreDatabaseZip($sourceDB)
	{
		if(file_exists($sourceDB))
		{
			...
			$f = fopen('temp/'.$this->getRealFileName($sourceDB).'.sql',"r+");
			$sqlFile = fread($f, filesize('temp/'.$this->getRealFileName($sourceDB).'.sql'));
